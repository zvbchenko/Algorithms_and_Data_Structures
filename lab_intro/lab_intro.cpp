...
PNG createSpotlight(PNG image, int centerX, int centerY) {
    for (unsigned x = 0; x < image.width(); x++) {
      for (unsigned y = 0; y < image.height(); y++) {
        double distance_from_center = sqrt((x-centerX)^2 +(y-centerY)^2);
        HSLAPixel *pixel = image.getPixel(x, y);
        double lumin = pixel->l;
        pixel->l = abs(1.0-distance_from_center*0.05)*lumin;
      }
    }
...
PNG ubcify(PNG image) {
    ...
    for (unsigned x = 0; x < image.width(); x++) {
      for (unsigned y = 0; y < image.height(); y++) { 
        HSLAPixel *pixel = image.getPixel(x, y);
        double hue_va = pixel->h;
        pixel->h = ((hue_va - 40) >= (hue_va - 210))? 210 : 40;
      }
    }
...
PNG watermark(PNG firstImage, PNG secondImage) {
  ...
  for (unsigned x = 0; x < firstImage.width(); x++) {
    for (unsigned y = 0; y < firstImage.height(); y++) { 
      ...
      double firstlu = firstpixel->l;
      double secondlu = secondpixel->l;
      firstpixel->h = (secondlu == 1 )? (firstlu+0.2) : firstlu;
    }
  }
